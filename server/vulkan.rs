...
impl Drop for VkCtx {
  fn drop(&mut self) {
    let mut debug_report_callbacks: HashMap<vk::Instance, Vec<vk::DebugReportCallbackEXT>> = 
    ...
    let mut device_pointers_map: HashMap<vk::Device, vk::DevicePointers> = 
      unsafe { std::mem::uninitialized() };
    ...
    let mut instance_pointers_map: HashMap<vk::Instance, vk::InstancePointers> =
      unsafe { std::mem::uninitialized() };
